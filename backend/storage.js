...
export async function addAppliedLog({ site, job_url, job_title = '', company = '', match_score = null }) {
  await db.run(
    'INSERT INTO applied (site, job_url, job_title, company, match_score) VALUES (?,?,?,?,?)',
    site, job_url, job_title, company, match_score
  );
}
export async function listAppliedLogs(limit = 100) {
  return db.all('SELECT * FROM applied ORDER BY created_at DESC LIMIT ?', limit);
}
