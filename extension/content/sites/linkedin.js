(function () {
  const { $all, click, sleep, text } = window.JAX;
  const {
    autoFillGeneric,
    ensureCoverLetterPdfAndPrompt,
  ...
  Sites.LinkedIn = {
    ...
    async applyIfEasy({ backend }) {
      ...
      if (!btn(/easy apply/i)) return false;
      await tap(/easy apply/i);

      for (let step = 0; step < 6; step++) {
        const jobDesc = (
          document.querySelector(
            "#job-details, .jobs-description__container, .show-more-less-html__markup"
          )?.innerText || ""
        ).slice(0, 5000);
        const role =
        ...
        const company =
          (
            document.querySelector(
              ".jobs-unified-top-card__company-name a, .jobs-unified-top-card__subtitle-primarily"
            )?.innerText || ""
          ).trim();
