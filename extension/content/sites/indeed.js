  ...
  Sites.Indeed = {
    ...
    async applyIfEasy({ backend }) {
      const btn = document.querySelector(
        'button:where([aria-label*="Apply" i],[id*="apply" i]), a:where([aria-label*="Apply" i])'
      );
      if (!btn) return false;
      ...
      const company = (
        document.querySelector(
          "[data-company-name], [data-testid=\"companyName\"]"
        )?.innerText || ""
      ).trim();
        ...
        try {
          ...
          for (const [label, val] of Object.entries(answers)) {
            const el = Array.from(
              ...
              (e) =>
                (e.getAttribute("aria-label") || "").includes(label) ||
                (e.id &&
      ...
      const submit = document.querySelector(
        'button:where([type="submit"], [aria-label*="Submit" i]):not([disabled])'
      );
      if (submit) {
        await click(submit);
        await sleep(1000);
