(function () {
  ...
  Sites.ZipRecruiter = {
    ...
    isListingPage() {
      return /jobs|search/i.test(location.pathname);
    },
    ...
    async applyIfEasy({ backend }) {
      ...
      ).filter((b) => /apply/i.test(text(b)));
      let applyBtn = candidates.find(
        (b) => !/company site|external/i.test(text(b))
      );
      ...
      const jobDesc = (
        document.querySelector(
          '[data-testid*="jobDescription"], #jobDescription'
        )?.innerText || ""
      ).slice(0, 5000);
      const role =
      ...
      const company =
        (
          document.querySelector(
            '[data-testid*="companyName"], a[href*="/company/"]'
          )?.innerText || ""
        ).trim();
      ...
      const submit = Array.from(document.querySelectorAll("button")).find(
        (b) => /submit|finish|send/i.test(text(b)) && !b.disabled
      );
      if (submit) {
        await click(submit);
        await sleep(1000);
