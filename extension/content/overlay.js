(function(){
if (window.__jobapplyx_overlay_injected) return; window.__jobapplyx_overlay_injected = true;
...
row.className = 'q';
row.innerHTML = `<label>${m.label || m.question}</label><input data-key="${m.key || m.label}" placeholder="Type answer"/>`;
qa.appendChild(row);
}
root.style.display = 'block';
return new Promise((resolve, reject) => {
root.querySelector('#ja-cancel').onclick = () => { root.style.display = 'none'; reject(new Error('cancel')); };
root.querySelector('#ja-send').onclick = () => {
const inputs = Array.from(qa.querySelectorAll('input,textarea'));
const answers = {}; for (const i of inputs) answers[i.getAttribute('data-key')] = i.value;
root.style.display = 'none'; resolve(answers);
};
});
},
async notice(html) {
...
send.onclick = () => { root.style.display = 'none'; resolve(true); };
});
}
};
})();